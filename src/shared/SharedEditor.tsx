...
import { uuid, type Member } from "./api";
import { paragraphDecorations } from "./paragraphDecorations";
...
export function paragraph(editor: Editor): ParagraphTarget | null {
...
export default function SharedEditor({ live, onComment, onReady, members = noMembers, selectedParagraph }: { live: LiveDocument; onComment: (target: ParagraphTarget | null) => void; onReady?: (editor: Editor) => void; members?: Member[]; selectedParagraph?: string }) {
  ...
  const people = useRef(members); people.current = members; const selected = useRef(selectedParagraph); selected.current = selectedParagraph;
  const comments = useMemo(() => paragraphDecorations(live, target => comment.current(target), () => people.current, () => selected.current), [live]);
  ...
  const editor = useEditor({ extensions: [StarterKit.configure({ undoRedo: false, heading: { levels: [1, 2, 3] }, link: { openOnClick: false, defaultProtocol: "https" } }),
    ...
    TableKit.configure({ table: { resizable: true } }), TextStyleKit, ToggleBlock, Columns, Column, Unsupported, Identity, CommentMark, NoteLink, comments,
  ...
  useEffect(() => { if (editor && !editor.isDestroyed) editor.view.dispatch(editor.state.tr.setMeta("comment-status", true)); }, [editor, members, selectedParagraph]);
