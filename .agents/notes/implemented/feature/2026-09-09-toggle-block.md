...
- `BlockLayout` 统一 28px 标记列、28px 每级缩进及祖先线位置；文字起点为编辑区 48px 基础前缀加层级缩进，折叠标题、待办和有序/无序列表另加标记列。正文为 15px，续行约 23px；折叠父子项起点间距 28px，普通顶层段落为 36px。拖动手柄命中、落点层级与箭头位置使用相同常量，不再各自硬编码旧缩进。待办复选框为 24×24px，列表标记以祖先线横坐标居中，避免与手柄重叠而无法打开删除菜单。
