...
原生拖放按命中行的上半/下半及标题中部判定时，父标题下沿被误当成整个子树之后；子树末尾与下一个标题共享的边界也有不同层级，鼠标跨过 1px 就会跳线或意外移出父级。只检查三级拖出与幽灵块尺寸无法发现内部排序的错误。单元格内还曾用提示线所依附段落的前缀绘制移动中的折叠项，导致层级正确但横向起点错误。
...
- 指针拖动的期望层级以实际按下手柄的位置为起点，每 28px 对应一层；三级向左一档成为二级同级。`BlockEditor` 以完整可见行的实际高度和滚动偏移定位边界，结构判定集中在 Core 的 `BlockDropTargets.cs#ResolveBlockDrop`。相邻行从上方或下方命中同一边界时，共用「下一块之前 / 前一块之后 / 前一折叠标题内部」候选；展开标题下沿是首个子项入口，只有子树真正结束的边界才提供祖先之后的位置。
- 纯纵向移动选取边界实际允许的最近层级，不在子项之间误跳到外层。明确向左拖动允许退出所处子树，提示线位于对应祖先最后一个可见后代之后；从源块原位置向右拖动可接入前一折叠项，已展开时接在可见子项末尾，收起时展开并插到首项。更深层级拖到其他折叠标题中部仍表示移入。先选层级，再用 `CanMove` 排除无效操作；当前位置的空移动不能回退到另一层级，循环移动不产生提示或历史。
- `BlockDropTarget` 同时携带提交目标、操作和提示线行/深度，松手用最终指针位置复算后调用同一 `Move`。线的横向起点按移动块在目的层级的 `PrefixInset` 计算，不能借用相邻段落的紧凑前缀。整个子树与收起状态保留，单次撤销恢复原层级。拖动预览使用 [幽灵块](2026-09-09-settings-ghost-opacity.md) 的只读原生排版，展开/收起分别反映可见子树/标题尺寸。
...
`BlockDropBoundaryTests` 的 19 项原生指针回归覆盖父标题/首项边界的 -1/0/+1px、长标题换行、子树末尾深度、同级排序、首项边界两侧左拖、原位右拖到展开/收起目标、空移动与循环拒绝，以及滚动和单元格/分栏中的提示线。测试读取实际绘制的蓝线像素，再验证松手后的顺序、父级、完整子树和一次撤销；不只断言落点记录。修复前首项用例把应在 28px 的提示线绘到 168px，尾部用例跨 1px 退出一级。验证图在 `artifacts/native/qa-toggle-drop-boundaries/`，不入 Git。
...
- 落点的像素几何留在 Desktop，候选层级和祖先边界留在 Core，增加了可见边界的维护约束；复用现有投影和移动事务，不新增持久化字段或撤销栈。提示线与实际结构需要成对回归，不能用单张拖动预览图代替。
