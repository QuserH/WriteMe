...
test("admin, two accounts, shared paragraph comments, menus and durable reconnect", async ({ page, browser, request }) => {
  ...
  await expect(page.getByRole("heading", { name: "账号管理" })).toBeVisible(); await expect(page.locator(".shared-sidebar")).toHaveCount(0);
  ...
  await expect(create).not.toBeVisible(); await expect(page.getByText("账号已创建，可以将登录信息交给伙伴。")).toBeVisible(); await page.screenshot({ path: "artifacts/shared-qa/screens/web-admin.png" });
  ...
  await page.getByRole("button", { name: "评论当前段落", exact: true }).click(); await page.getByRole("textbox", { name: "评论内容" }).fill("这段思路可以再补一个具体例子。"); await page.getByRole("button", { name: /发送 ↑/ }).click();
  await peer.getByRole("button", { name: /^评论/ }).filter({ hasText: "评论" }).first().click(); await expect(peer.getByText("这段思路可以再补一个具体例子。", { exact: true })).toBeVisible();
  await peer.getByRole("button", { name: "回复", exact: true }).first().click(); await peer.getByRole("textbox", { name: "评论内容" }).fill("好，我来补上用户场景。"); await peer.getByRole("button", { name: /发送 ↑/ }).click();
  await expect(page.getByText("好，我来补上用户场景。", { exact: true })).toBeVisible(); await page.getByRole("button", { name: "回复", exact: true }).last().click(); await page.getByRole("textbox", { name: "评论内容" }).fill("收到，我们接着这条回复讨论。"); await page.getByRole("button", { name: /发送 ↑/ }).click(); await expect(peer.getByText("收到，我们接着这条回复讨论。", { exact: true })).toBeVisible();
